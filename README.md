...
- **分类导航** — 顶部分类拆分为：注册 / 账号管理 / 邮箱服务 / 支付绑卡 / 设置
- **邮箱服务中心化** — 邮箱配置统一在「邮箱服务」管理，注册与支付页面只做引用
- **账号管理增强** — 账号列表支持订阅状态识别（基于 Stripe confirm 结果），避免仅按执行状态误判
- **设置页可配置** — 新增代理列表管理与注册重试默认值管理
...
> 建议首次启动后在 Web UI 的「邮箱服务」中导入/维护邮箱服务配置，后续注册与支付直接选择服务。
...
`publish-ghcr.yml` 的默认策略：

- 推送到 `main`：只构建 `linux/amd64`（更快，适合频繁迭代）
- 打 `v*` 标签：构建 `linux/amd64,linux/arm64`（发版用）
- 手动触发 `workflow_dispatch`：可自定义 `platforms`
...
2. **设置**：到「设置」维护代理列表（可设默认代理）和注册重试默认值
3. **邮箱服务**：到「邮箱服务」新增或导入 `mail_worker` 服务并启用
4. **注册**：到「注册」分类选择邮箱服务与代理，可配置全流程/OTP/Session 重试参数
5. **支付绑卡**：选择新注册或已有账号，填写卡片与账单信息
6. **执行**：点击开始执行，进度条显示步骤，完成后在「账号管理」查看订阅状态
